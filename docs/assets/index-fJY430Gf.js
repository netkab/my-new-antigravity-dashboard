(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const s of a)if(s.type==="childList")for(const t of s.addedNodes)t.tagName==="LINK"&&t.rel==="modulepreload"&&n(t)}).observe(document,{childList:!0,subtree:!0});function i(a){const s={};return a.integrity&&(s.integrity=a.integrity),a.referrerPolicy&&(s.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?s.credentials="include":a.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function n(a){if(a.ep)return;a.ep=!0;const s=i(a);fetch(a.href,s)}})();const y={metaTitle:"Minoris Dashboard",sidebar:`
      <div class="logo-area">
  ...
  `,tabs:{liveIncidents:`
    ...
    `}},u=(o,e)=>{const i=document.querySelector(o);i&&(i.innerHTML=e.trim())};document.addEventListener("DOMContentLoaded",()=>{document.title=y.metaTitle,u(".sidebar",y.sidebar),u("#live-incidents",y.tabs.liveIncidents),u("#advanced-analytics",y.tabs.advancedAnalytics),u("#strategy",y.tabs.strategy),u("#investigation",y.tabs.investigation),b(),k(),A(),L(),w()});const b=()=>{const o=document.querySelectorAll(".nav-item"),e=document.querySelectorAll(".tab-content");o.forEach(i=>{i.addEventListener("click",n=>{n.preventDefault(),o.forEach(t=>t.classList.remove("active")),i.classList.add("active"),e.forEach(t=>t.classList.remove("active"));const a=i.getAttribute("data-tab"),s=a?document.getElementById(a):null;s&&s.classList.add("active")})})},k=()=>{document.querySelectorAll(".alert-header-row").forEach(e=>{e.addEventListener("click",()=>{const i=e.parentElement;if(!i)return;const n=i.classList.contains("active"),a=e.querySelector(".chevron");n?(i.classList.remove("active"),a&&(a.textContent="▶")):(i.classList.add("active"),a&&(a.textContent="▼"))})})},w=()=>{document.querySelectorAll("#live-incidents .alert-body").forEach(e=>{if(e.querySelector(".incident-action-stack"))return;const i=document.createElement("div");i.className="incident-action-stack";const n=document.createElement("div");n.className="incident-actions";const a=document.createElement("button");a.type="button",a.className="incident-btn acknowledge",a.textContent="Acknowledge";const s=document.createElement("button");s.type="button",s.className="incident-btn dismiss",s.textContent="Dismiss",n.appendChild(a),n.appendChild(s);const t=document.createElement("form");t.className="decision-form hidden",t.dataset.action="Acknowledge";const d=document.createElement("div");d.className="decision-label",d.textContent="Add note for Acknowledge";const r=document.createElement("textarea");r.className="decision-note",r.placeholder="Log the context behind this decision...";const h=document.createElement("button");h.type="submit",h.className="submit-btn",h.textContent="Submit";const v=document.createElement("div");v.className="form-status hidden",v.textContent="Decision recorded.",t.append(d,r,h,v),i.append(n,t);const l=e.querySelector(".cards-container");l?l.insertAdjacentElement("afterend",i):e.appendChild(i);const f=c=>{t.dataset.action=c,d.textContent=`Add note for ${c}`,r.placeholder=c==="Acknowledge"?"What context led to acknowledging this incident?":"Why should this be dismissed? Include guardrails.",t.classList.remove("hidden"),v.classList.add("hidden"),r.focus()};a.addEventListener("click",()=>f("Acknowledge")),s.addEventListener("click",()=>f("Dismiss")),t.addEventListener("submit",c=>{c.preventDefault();const g=t.dataset.action||"Acknowledge",p=r.value.trim();v.textContent=p?`${g} logged with note: "${p}"`:`${g} logged.`,v.classList.remove("hidden"),r.value=""})})},A=()=>{const o=document.getElementById("analysis-form"),e=document.getElementById("analysis-input"),i=document.getElementById("analysis-spinner"),n=document.getElementById("analysis-output"),a=document.getElementById("analysis-query-bubble");let s;!o||!e||!i||!n||!a||o.addEventListener("submit",t=>{t.preventDefault();const d=e.value.trim();d&&(a.textContent=d,i.classList.remove("hidden"),n.classList.add("hidden"),s&&window.clearTimeout(s),s=window.setTimeout(()=>{i.classList.add("hidden"),n.classList.remove("hidden")},1e3))})},L=()=>{document.querySelectorAll(".chart-interactive").forEach(e=>{const i=e.dataset.points;if(!i)return;const n=e.querySelector(".chart-cursor"),a=e.querySelector(".chart-hover-line"),s=e.querySelector(".chart-hit-area"),t=e.closest(".chart-container")?.querySelector('[data-role="chart-value"]');if(!n||!a||!s)return;const d=t?.textContent??"",r=i.split("|").map(l=>{const[f,c,g="",p=""]=l.split(","),x=Number(f),m=Number(c);return{x,y:m,label:g,value:p}}).filter(l=>!Number.isNaN(l.x)&&!Number.isNaN(l.y));if(!r.length)return;const h=l=>{n.setAttribute("cx",l.x.toString()),n.setAttribute("cy",l.y.toString()),a.setAttribute("x1",l.x.toString()),a.setAttribute("x2",l.x.toString()),a.setAttribute("y1","20"),a.setAttribute("y2","190"),t&&(t.textContent=`${l.label} → ${l.value}`)},v=l=>{const f=e.getBoundingClientRect(),c=l-f.left,g=r.reduce((p,x)=>{const m=Math.abs(p.x-c);return Math.abs(x.x-c)<m?x:p});n.classList.remove("hidden"),a.classList.remove("hidden"),h(g)};s.addEventListener("pointerenter",l=>v(l.clientX)),s.addEventListener("pointermove",l=>v(l.clientX)),e.addEventListener("pointerleave",()=>{n.classList.add("hidden"),a.classList.add("hidden"),t&&(t.textContent=d)})})};
